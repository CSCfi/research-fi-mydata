...
xml_folder = 'xml/'
xml_files = [
    #{ "name": "xmlJulkaisut.xml", "tag": "<Julkaisu>" },
    #{ "name": "Meriitit.xml", "tag": "<Meriitti>" },
    #{ "name": "Projektit.xml", "tag": "<Projekti>" },
    { "name": "Tutkijat.xml", "tag": "<Tutkija>" },
    #{ "name": "Tutkimusaineistot.xml", "tag": "<Tutkimusaineisto>" },
]
count_new = 0
count_update = 0

for xml_file in xml_files:
    filename = xml_folder + xml_file["name"]
    print("-----")
    print(filename)
    with open(filename) as infile:
        for line in infile:
            ...
            if line.startswith(xml_file["tag"]):
